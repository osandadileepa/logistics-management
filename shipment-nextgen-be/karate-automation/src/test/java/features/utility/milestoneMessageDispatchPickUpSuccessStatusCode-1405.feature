Feature: Simulate Milestone Message from Dispatch
  ...
  @SimulateMilestoneMessageWithSuccessPickupStatusCodeFromDispatch
  @Utility
  @Regression
  Scenario: Simulate Milestone Message With Success Pick Up Status Code from Dispatch
    ...
    * def segmentId1 = data.shipment_journey.package_journey_segments[0].segment_id
    * match data.shipment_journey.package_journey_segments[0].status == 'PLANNED'
    ...
    * milestoneMessage.milestone_code = '1405'
    * milestoneMessage.create_time = currentTime
    * milestoneMessage.milestone_time = currentTime
    * def pickupActualTime = milestoneMessage.milestone_time
    ...
    * def response = $
    * def data = response.data
    * match data.milestone_events[0].milestone_code == '1405'
    * match data.shipment_journey.package_journey_segments[0].pick_up_actual_time == '#notnull'
    * match data.shipment_journey.package_journey_segments[0].pick_up_actual_timezone == '#notnull'
