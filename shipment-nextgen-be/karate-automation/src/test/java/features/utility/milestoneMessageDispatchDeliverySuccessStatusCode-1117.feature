Feature: Simulate Milestone Message from Dispatch
  ...
  @SimulateMilestoneMessageWithSuccessDeliveryStatusCodeFromDispatch
  @Utility
  @Regression
  Scenario: Simulate Milestone Message With Success Delivery Status Code from Dispatch

    * def milestoneMessage = read('classpath:features/utility/json/milestoneMessageForSuccessDelivery.json')
    ...
    * milestoneMessage.milestone_code = '1117'
    ...
    * match data.shipment_journey.package_journey_segments[0].drop_off_actual_time == '#notnull'
    * match data.shipment_journey.package_journey_segments[0].drop_off_actual_timezone == '#notnull'
