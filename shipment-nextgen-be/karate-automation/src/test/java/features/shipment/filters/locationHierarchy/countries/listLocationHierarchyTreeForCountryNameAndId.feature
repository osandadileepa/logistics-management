Feature: List Location Hierarchy Trees With Country Nodes
  ...
  Scenario: List Location Hierarchy Trees With Country Nodes Given Country Name and Country Id
    * def bearer = token
    Given url baseUrl + '/filter/location_hierarchies?country_id=' + countryId + '&key=' + countryName + '&per_page=10&page=1&level=1'
