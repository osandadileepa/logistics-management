Feature: List Location Hierarchy Trees With Country Nodes
  ...
  Scenario: List Location Hierarchy Trees With Country Nodes Given Country Id
    * def bearer = token
    Given url baseUrl + '/filter/location_hierarchies?country_id=' + countryId + '&per_page=10&page=1&level=1'
