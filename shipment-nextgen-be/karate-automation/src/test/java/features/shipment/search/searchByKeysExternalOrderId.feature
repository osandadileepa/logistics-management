Feature: Search by External Order Id Keys Feature
  ...
  Background:
    ...
    * def findAllRequest = read('classpath:features/shipment/json/search/keysExternalOrderIdsRQ.json')
    * print findAllRequest

  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys external order id keys
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys lower case external order id keys
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys upper case external order id keys
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys unknown external order id keys
    * def bearer = token
