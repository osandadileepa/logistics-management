Feature: Search by Customer Order Id Keys Feature
  ...
  Background:
    ...
    * def findAllRequest = read('classpath:features/shipment/json/search/keysCustomerOrderIdsRQ.json')
    * print findAllRequest

  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys customer order id
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys lower case customer order id
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys upper case customer order id
    * def bearer = token
  ...
  @ShipmentFindAll @SegmentRegression
  Scenario: Find All Shipments with keys unknown customer order id
    * def bearer = token
    * findAllRequest.data.keys[0] = 'unknown_master_way_bill_1'
    * findAllRequest.data.keys[1] = 'unknown_master_way_bill_2'
    Given path '/shipments/list'
    And header Authorization = 'Bearer '+ bearer
    And request findAllRequest
    When method POST
