Feature: Search by Order Feature
  ...
  Background:
    ...
    * def orderId = createBulkShipmentResult.data[0].shipment.order.id
    * def findAllRequest = read('classpath:features/shipment/json/search/orderRQ.json')
    * findAllRequest.data.order.id = orderId
  ...
  Scenario: Find All Shipments
    ...
    * match data.result == '#[1]'
    * match data.filter.order.id == orderId
