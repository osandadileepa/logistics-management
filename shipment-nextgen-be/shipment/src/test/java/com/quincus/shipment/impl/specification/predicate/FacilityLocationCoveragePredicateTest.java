...
import com.quincus.shipment.impl.repository.specification.predicate.FacilityLocationCoveragePredicate;
...
class FacilityLocationCoveragePredicateTest {

    private final ShipmentLocationCoveragePredicate predicate = new FacilityLocationCoveragePredicate();
    @Mock
    ...
    @Test
    @DisplayName("GIVEN UserLocationCoverage has no Facility assigned WHEN building predicates THEN return should be null")
    void givenNoUserCoverageForFacility_WhenConstructPredicate_ThenShouldReturnNull() {
        //WHEN:
    ...
    @Test
    @DisplayName("GIVEN UserLocationCoverage has  Facility assigned WHEN building predicates THEN return should be null")
    void givenUserHasCoverageForFacility_WhenConstructPredicate_ThenShouldReturnPredicate() {
        //GIVEN:
        when(userLocationCoverageByType.get(LocationType.FACILITY)).thenReturn(Arrays.asList("1", "2"));
        when(locationHierarchyJoin.join(LocationHierarchyEntity_.facility)).thenReturn(locationJoin);
