...
import com.quincus.shipment.impl.repository.specification.predicate.CountryLocationCoveragePredicate;
...
class CountryLocationCoveragePredicateTest {

    private final ShipmentLocationCoveragePredicate predicate = new CountryLocationCoveragePredicate();
    @Mock
    ...
    @Test
    @DisplayName("GIVEN UserLocationCoverage has no Country assigned WHEN building predicates THEN return should be null")
    void givenNoUserCoverageForCountry_WhenConstructPredicate_ThenShouldReturnNull() {
        //WHEN:
    ...
    @Test
    @DisplayName("GIVEN UserLocationCoverage has  Country assigned WHEN building predicates THEN return should be null")
    void givenUserHasCoverageForCountry_WhenConstructPredicate_ThenShouldReturnPredicate() {
        //GIVEN:
        when(userLocationCoverageByType.get(LocationType.COUNTRY)).thenReturn(Arrays.asList("1", "2"));
        when(locationHierarchyJoin.join(LocationHierarchyEntity_.country)).thenReturn(locationJoin);
