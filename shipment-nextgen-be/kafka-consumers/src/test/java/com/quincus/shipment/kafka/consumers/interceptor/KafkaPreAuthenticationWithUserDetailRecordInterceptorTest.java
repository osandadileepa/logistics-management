...
class KafkaPreAuthenticationWithUserDetailRecordInterceptorTest {
    private static final String TEST_MESSAGE = "{\"organisation_id\": \"test-org-id\", \"id\": \"test-order-id\",\"user_id\": \"test-user-id\"}";
    ...
    @InjectMocks
    private KafkaPreAuthenticationWithUserDetailRecordInterceptor interceptor;

    @Test
    void testIntercept_ValidOrgIdAndUserId() {
        ConsumerRecord<String, String> record = new ConsumerRecord<>("test-topic", 0, 0, "test-key", TEST_MESSAGE);
        when(authentication.isAuthenticated()).thenReturn(true);
        when(authentication.getCredentials()).thenReturn("KAFKA");
        when(authenticationProvider.authenticatePreAuthenticatedUserWithUserId(anyString(), anyString(), anyString())).thenReturn(authentication);
    ...
    @Test
    void testIntercept_InvalidUserId() {
        ConsumerRecord<String, String> record = new ConsumerRecord<>("test-topic", 0, 0, "test-key", TEST_MESSAGE);
    ...
    void testInterceptWithInvalidMessage() {
        ...
        interceptor.intercept(record);
        verify(authenticationProvider, never()).authenticatePreAuthenticatedUserWithUserId(anyString(), anyString(), anyString());
        assertThat(SecurityContextHolder.getContext().getAuthentication()).isNull();
    }
    ...
    void testOnConsume() {
        String key = "key";
        String value = "{\"organisation_id\":\"123\",\"id\":\"456\",\"user_id\":\"user_1\"}";
        int offset = 0;
