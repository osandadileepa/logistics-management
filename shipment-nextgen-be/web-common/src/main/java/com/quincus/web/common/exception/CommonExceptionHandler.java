...
public class CommonExceptionHandler {
    ...
    public ResponseEntity<Response<String>> handleApiCallException(ApiCallException e) {
        ...
        return new ResponseEntity<>(response, filterErrorStatusResponse(e.getResponseStatus()));
    }

    @ExceptionHandler
    public ResponseEntity<Response<String>> handleApiNetworkIssueException(ApiNetworkIssueException e) {
        log.error(ERR_API_CALL, e.getMessage(), e);
        Response<String> response = new Response<>(null);
        response.setMessage(e.getMessage());
        return new ResponseEntity<>(response, filterErrorStatusResponse(e.getResponseStatus()));
    ...
    private ResponseEntity<Response<QuincusError>> buildResponse(QuincusError shipmentError, HttpStatus httpStatus) {
        Response<QuincusError> response = new Response<>(shipmentError);
        response.setMessage(shipmentError.message());
        response.setStatus(String.valueOf(httpStatus.value()));
        return new ResponseEntity<>(response, httpStatus);
    }

    private HttpStatus filterErrorStatusResponse(HttpStatus errorStatus) {
        return errorStatus == HttpStatus.INTERNAL_SERVER_ERROR ? HttpStatus.UNPROCESSABLE_ENTITY : errorStatus;
    }
}
