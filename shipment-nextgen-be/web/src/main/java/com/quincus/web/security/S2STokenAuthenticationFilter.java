...
import com.quincus.web.CacheHttpServletRequestWrapper;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            if (isAuthenticationNotSet() && StringUtils.isNotBlank(organizationId) && token.isPresent()) {
                // We need this custom request wrapper as reading input stream will clear the request body
                request = new CacheHttpServletRequestWrapper(request);
                String payloadUserId = tokenResolver.resolveUserId(request);
                Authentication authentication;
                if (StringUtils.isNotBlank(payloadUserId)) {
                    authentication = authenticationProvider.authenticatePreAuthenticatedUserWithUserId(organizationId,
                            payloadUserId, S2S);
                } else {
                    authentication = authenticationProvider.authenticatePreAuthenticatedUser(organizationId
                            , S2S, token.get().getName());
                }
