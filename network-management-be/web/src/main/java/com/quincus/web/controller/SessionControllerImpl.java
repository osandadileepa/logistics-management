...
public class SessionControllerImpl implements SessionController {
    ...
    public Response<AuthenticationUser> create(final AuthenticationRequest authenticationRequest) {
        if (StringUtils.isNotBlank(authenticationRequest.getToken())) {
            return new Response<>(authenticationClient.validateToken(authenticationRequest.getToken()));
        }
        if (StringUtils.isNotBlank(authenticationRequest.getUsername()) && StringUtils.isNotBlank(authenticationRequest.getPassword())) {
            return new Response<>(authenticationClient.login(authenticationRequest.getUsername(),
