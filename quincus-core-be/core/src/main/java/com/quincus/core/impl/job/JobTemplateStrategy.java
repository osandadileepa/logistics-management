...
import com.quincus.core.impl.exception.JobCancelledException;
import com.quincus.core.impl.exception.JobRecordExecutionException;
...
public abstract class JobTemplateStrategy<V> {
    ...
    private void handleCancelledJob(JobMetrics<V> jobMetrics, JobCancelledException e) {
    ...
    private void updateJobMetricsOnBatchProcessing(JobMetrics<V> jobMetrics) {
        ...
        if (JobState.CANCELLED == jobMetricsService.getJobMetricsByJobId(jobMetrics.getId(), jobMetrics.getOrganizationId()).getStatus()) {
            ...
            throw new JobCancelledException(String.format(JOB_CANCELLED_ERROR_MESSAGE, jobMetrics.getId()));
        }
    ...
    public abstract void execute(V data) throws JobRecordExecutionException;
}